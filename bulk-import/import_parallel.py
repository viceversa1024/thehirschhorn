...
def upload_files(row):
    ...
    try:
        subprocess.run(["aws", "s3", "cp", "--acl=public-read", f"./photos/IMG_{row['front']}.jpeg", f"s3://{BUCKET_NAME}/{barcode}.front.jpg"], check=True)
        subprocess.run(["aws", "s3", "cp", "--acl=public-read", f"./photos/IMG_{row['back']}.jpeg", f"s3://{BUCKET_NAME}/{barcode}.back.jpg"], check=True)
